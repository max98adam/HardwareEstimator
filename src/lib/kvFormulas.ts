...
export const KV_FORMULA_DETAILS: Record<KvFormula, KvFormulaInfo> = {
  standard: {
    label: "Standard GQA",
    models: "Llama 3, Qwen 2.5/3, Mistral, Phi, GLM, MiniMax",
  ...
  hybrid: {
    ...
    models: "Gemma 2 / 3 / 4, gpt-oss, Llama 4, DeepSeek V4 (approx.)",
    formula: "sliding_layers × … × min(T, W) + full_layers × … × T",
    note: "Local-attention layers cap memory at W (sliding window, e.g. 4096; gpt-oss 128; Llama 4 chunked 8192). Only every Nth layer keeps a full cache. Gemma 4 dense 31B and the MoE 26B variant share K and V storage, halving the KV cache.",
  },
  mla: {
    label: "MLA",
    models: "DeepSeek V2 / V3 / V3.2, Kimi K2",
  ...
  linear_hybrid: {
    label: "Linear + Full",
    models: "Qwen 3.5 / 3.6 / 3-Next, Kimi-Linear, MiniMax-M1",
